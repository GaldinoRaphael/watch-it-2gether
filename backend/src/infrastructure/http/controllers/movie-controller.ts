...
export class MovieController {
  ...
  async search(req: Request, res: Response) {
    try {
      ...
      const movies = await this.searchMoviesUseCase.execute(typeof query === "string" ? query : "");
      return res.json(movies);
    } catch (error) {
      return res
        .status(500)
        .json({ error: "Internal Server Error", errorMessage: (error as Error).message });
    }
  }
  ...
  async getStoredMovieByExternalId(req: Request, res: Response) {
    try {
      const movie = await this.movieRepositoryUseCase.getByExternalId(
  ...
  async updateStoredMovie(req: Request, res: Response) {
    try {
      const movie = await this.movieRepositoryUseCase.update(
        String(req.params.id),
        req.body as UpdateMovieInput,
      );
      return res.json(movie);
    } catch (error) {
      return this.handleError(res, error);
